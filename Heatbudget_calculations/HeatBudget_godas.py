...
def get_temp_tendency(model):
    #load in T
    T = xr.open_mfdataset(model, parallel=True).pottmp.rename({'level':'lev'})
...
def load_U(model):
    #load in U
    U = xr.open_mfdataset(model, parallel=True).ucur.rename({'level':'lev'})
    #convert level to m if not already
...
def load_V(model):
    #load in U
    U = xr.open_mfdataset(model, parallel=True).vcur.rename({'level':'lev'})
    if U.lev.attrs['units'] =='m':
        pass
    elif U.lev.attrs['units'] =='cm':
        U['lev'] = U['lev']/100
...
def load_W(model):
    #load in W
    W = xr.open_mfdataset(model, parallel=True).dzdt.rename({'level':'lev'})
    if W.lev.attrs['units'] =='m':
        pass
    elif W.lev.attrs['units'] =='cm':
        W['lev'] = W['lev']/100
        W.lev.attrs['units'] = 'm'
    elif W.lev.attrs['units'] =='centimeters':
        W['lev'] = W['lev']/100
        W.lev.attrs['units'] = 'm'
    #subset spatially
    W = W.sel(lat=slice(-40,30),lon=slice(30,125))
    #subset time
    W = W.sel(time= slice('1950', '2015'))
    #set land values to 0
    W = W.fillna(0)
    return W
...
def load_hfds(model):
    q = xr.open_mfdataset(model, parallel=True).thflx
    #subset spatially
...
def load_rsntds(model):
    r = xr.open_mfdataset(model, parallel=True).dswrf
    #subset spatially
