...
def get_temp_tendency(model):
    #load in T
    T = xr.open_mfdataset(model, parallel=True).temp.rename({'st_ocean':'lev'})
...
def load_U(model):
    #load in U
    U = xr.open_mfdataset(model, parallel=True).u.rename({'st_ocean':'lev'})
    #convert level to m if not already
...
def load_V(model):
    #load in U
    U = xr.open_mfdataset(model, parallel=True).v.rename({'st_ocean':'lev'})
    if U.lev.attrs['units'] =='m':
        pass
    elif U.lev.attrs['units'] =='cm':
        U['lev'] = U['lev']/100
...
def load_hfds(model):
    q = xr.open_mfdataset(model, parallel=True).net_heating
    #subset spatially
    q = q.sel(lat=slice(-40,30),lon=slice(30,125))
    #subset time
    q = q.sel(time= slice('1950', '2018'))
    return q
